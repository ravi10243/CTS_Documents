({
    ...
    fetchCustomerType : function(component, event) {
        ...
        customerTypeConfig.sort((a, b) => (a.Category_AGN__c > b.Category_AGN__c) ? 1 : -1); //Added by Ravi for soting the Records order by Sub category
        ...
        if(selected != undefined && selected != null && selected != ''){
            ...
            if ($A.util.isArray(customerTypeConfig)){               
                customerTypeConfig.forEach(function(currRow){  
                    if(selected.toUpperCase() ==='AN' || selected.toUpperCase() ==='NZ'){                       
    ...
    fetchCustomerSubType : function(component, event) {
        var newlst = [];
        var customerTypeConfig = component.get("v.customerTypeConfig");        
        customerTypeConfig.sort((a, b) => (a.Sub_Category__c > b.Sub_Category__c) ? 1 : -1); //Added by Ravi for soting the Records order by Sub category
                ...
                customerTypeConfig.forEach(function(currRow){                    
                    if(currRow.Category_AGN__c.toUpperCase() === selected.toUpperCase()){
                        if (newlst.map(function(e) { return e.key; }).indexOf(currRow.Sub_Category__c) <0){ // duplicate remove
                            ...
                            if(selectedCountry === 'AN' || selectedCountry === 'NZ'){
    ...
    fireEvent : function(component, event) {
        ...
        console.log("selected country>>>>>>header>>>>>"+component.get("v.selectedCountry"));
