({
    ...
    MAX_DOC_FILE_SIZE: 750000, /* 1 000 000 * 3/4 to account for base64 */
    ...
    validateAddressRequiredFields : function(component){
      ...
      /*  if((jQuery.trim(jQuery(this).val())!='')&&(jQuery(this).hasClass('reg_phone'))){
        ...
        if (requiredMissing == true){                
            message=$A.get("$Label.c.AGN_OAM_Body_PleaseFill"); 
            //alert('required Missing') ;
            //return false;
        }
                   
        if(wrongPostCodeFormat){
            message= message+$A.get("$Label.c.AGN_OAM_Body_PleaseCheckFormatFor")+ $A.get("$Label.c.AGN_OAM_Body_PostalCode")+','; 
            //label_content();
        }
        
        if(wrongEmailFormat){
            //alert('required email') ;
            if(message.slice(-1)!= ','){message= message+$A.get("$Label.c.AGN_OAM_Body_PleaseCheckFormatFor")+ $A.get("$Label.c.AGN_OAM_Body_EmailAddress")+'';}
            else message= message+$A.get("$Label.c.AGN_OAM_Body_EmailAddress")+',';
           // label_content();
        }
        ...
        if(!requiredMissing && !wrongPostCodeFormat && !wrongEmailFormat) {
           //alert('final block');
