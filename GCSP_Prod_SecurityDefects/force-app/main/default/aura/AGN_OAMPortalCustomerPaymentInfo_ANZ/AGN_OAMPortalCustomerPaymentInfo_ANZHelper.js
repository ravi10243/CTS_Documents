({
    ...
    fetchCustomerDetails : function(component, event) {
        ...
        action.setCallback(this, function(response) {
            if(response.getState() === 'SUCCESS') {  
                ...
                // console.log('Sobject-->'+component.get("v.customerGroup"));   
    ...
    onChangePaymentTerm: function(component, event, helper){ 
        ...
        var secVal = [];            
        if(!$A.util.isEmpty(pmethod) && !$A.util.isEmpty(pTerm)){
            ...
            if(pName.toUpperCase() === $A.get("$Label.c.AGN_OAM_30Days_End_Of_Month")){ //'30 days credit account'
    ...
    validateAndUpsert : function(component, event) {
      component.set("v.showSpinner", true);
        ...
        if(!cmpfield && !$A.util.isEmpty(paymentTerm) && !$A.util.isEmpty(pmethod)){
	     console.log('PT Value>>>>>>>>>>>>>>>>>'+paymentTerm);            
         console.log('PM Value>>>>>>>>>>>>>>>>>'+pmethod);   
            var self = this; 
            ...
            console.log('cmpSBSRequired>>>>>>>>>>>>>'+JSON.stringify(cmpSBS));
            if (!cmpSBS) return;
    ...
    validateFormatFields: function(component, event) {
       console.log('Inside check formate');
        jQuery.noConflict();
